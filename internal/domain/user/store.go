...
func (s *Store) GetUserByUsername(ctx context.Context, username string) (model.User, error) {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return model.User{}, op.Err(domain.Error(domain.CodeNotFound, "user not found", err))
		}

		return model.User{}, op.Err(domain.Error(domain.CodeInternal, err.Error(), err))
	}
...
func (s *Store) toDomainUser(u dbstore.User) model.User {
	return model.User{
		ID:           u.ID,
		Username:     u.Username,
