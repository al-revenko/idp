...
func (s *Store) GetClientById(ctx context.Context, id string) (model.Client, error) {
	op := pkg.Op("Store.GetClientById")

	c, err := s.db.GetClientById(ctx, id)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return model.Client{}, op.Err(domain.Error(domain.CodeNotFound, "client not found", err))
		}

		return model.Client{}, op.Err(domain.Error(domain.CodeInternal, err.Error(), err))
	}
