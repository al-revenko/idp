...
func (e ValidationError) Error() string {
	...
	if errors.As(e.cause, &ve) {
		...
		for i, fe := range ve {
			...
			} else if fe.Tag() == "password" {
				builder.WriteString(fmt.Sprintf("%s: should be at least 8 characters and include an uppercase letter, a lowercase letter, a number, and a special character%s", field, endOfmsg))
			} else if fe.Param() != "" {
