...
func ErrorInterceptor(log *slog.Logger) grpc.UnaryServerInterceptor {
	return func(ctx context.Context, req any, info *grpc.UnaryServerInfo, handler grpc.UnaryHandler) (any, error) {
		...
		if !isExpectedError(err) {
			if e, ok := errors.AsType[domain.DomainError](err); ok && e.Cause != nil {
...
func isExpectedError(err error) bool {
	if e, ok := errors.AsType[domain.DomainError](err); ok {
		return e.Code != domain.CodeInternal
	}

	if _, ok := errors.AsType[valid.ValidationError](err); ok {
		return true
	}
